...
<Callout>
That route handles only the 🧠 reasoning-effort dropdown, which a reaction cannot express. It
verifies the signature and runs the **same `isAllowed` gate** as a mention before acting on
any payload.
</Callout>

Emoji controls also need the `reactions:read` scope and the `reaction_added` bot event, both
...
Set these in `wrangler.jsonc` as comma-separated Slack IDs:
