...
1. resolves a permalink to the source thread, then posts a **new top-level message** in
   the same channel announcing who forked it and linking back;
...
5. rewrites that announcement to name the new box, which is only known after the fork;
6. links the new thread from the old one.
